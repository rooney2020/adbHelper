...
function registerIpcHandlers() {
    ipcMain.handle("settings.setUseEmbeddedBrowser", async (_event, value) => {
        ...
        try {
            ...
            if (result.isBinary) {
                try {
                    ...
                    result.content = `[二进制文件，无法文本显示] (${(Buffer.from(result.content, "base64").length / 1024).toFixed(1)} KB)`;
                }
            }
            else {
                try {
    ...
    ipcMain.handle("trace.start", async (_event, payload) => {
        try {
            ...
            await execFileAsync("adb", ["-s", payload.deviceId, "shell", "atrace", "--async_start", "-b", "40960", ...categories], { timeout: 10000 });
            await new Promise((r) => setTimeout(r, duration * 1000));
            await execFileAsync("adb", ["-s", payload.deviceId, "shell", "atrace", "--async_stop", "-o", remotePath], { timeout: 30000 });
    ...
    ipcMain.handle("trace.listFiles", async () => {
        try {
            ...
            const files = [];
            for (const entry of entries) {
                if (!entry.endsWith(".perfetto-trace"))
                    continue;
                const fullPath = join(outputDir, entry);
                try {
                    const stat = await fsStat(fullPath);
    ...
    const monkeySessions = new Map();
    ipcMain.handle("monkey.start", async (_event, payload) => {
        try {
            ...
            if (config.includePackages) {
                for (const pkg of config.includePackages) {
                    if (pkg.trim())
                        monkeyArgs.push("-p", pkg.trim());
                }
            }
            if (config.throttle > 0)
                monkeyArgs.push("--throttle", String(config.throttle));
            if (config.pctTouch > 0)
                monkeyArgs.push("--pct-touch", String(config.pctTouch));
            if (config.pctMotion > 0)
                monkeyArgs.push("--pct-motion", String(config.pctMotion));
            if (config.pctTrackball > 0)
                monkeyArgs.push("--pct-trackball", String(config.pctTrackball));
            if (config.pctNav > 0)
                monkeyArgs.push("--pct-nav", String(config.pctNav));
            if (config.pctMajornav > 0)
                monkeyArgs.push("--pct-majornav", String(config.pctMajornav));
            if (config.pctSyskeys > 0)
                monkeyArgs.push("--pct-syskeys", String(config.pctSyskeys));
            if (config.pctAppswitch > 0)
                monkeyArgs.push("--pct-appswitch", String(config.pctAppswitch));
            if (config.pctFlip > 0)
                monkeyArgs.push("--pct-flip", String(config.pctFlip));
            if (config.pctAnyevent > 0)
                monkeyArgs.push("--pct-anyevent", String(config.pctAnyevent));
            if (config.seed)
                monkeyArgs.push("-s", config.seed);
            if (config.verbosity > 0) {
                for (let i = 0; i < config.verbosity; i++)
                    monkeyArgs.push("-v");
            }
            if (config.ignoreCrashes)
                monkeyArgs.push("--ignore-crashes");
            if (config.ignoreTimeouts)
                monkeyArgs.push("--ignore-timeouts");
            if (config.ignoreSecurityExceptions)
                monkeyArgs.push("--ignore-security-exceptions");
            if (config.ignoreNativeCrashes)
                monkeyArgs.push("--ignore-native-crashes");
            if (config.killProcessAfterError)
                monkeyArgs.push("--kill-process-after-error");
            if (config.monitorNativeCrashes)
                monkeyArgs.push("--monitor-native-crashes");
            ...
            const appendLog = (line) => {
                ...
                if (session.logBuffer.length > MONKEY_LOG_LIMIT)
                    session.logBuffer.shift();
                const evMatch = line.match(MONKEY_EVENT_PATTERN);
                if (evMatch)
                    session.completedEvents = parseInt(evMatch[1], 10);
    ...
    ipcMain.handle("monkey.stop", async (_event, payload) => {
        try {
            ...
            try {
                ...
                for (const line of monkeyLines) {
                    ...
                    if (pid)
                        await execFileAsync("adb", ["-s", deviceId, "shell", "kill", "-9", pid], { timeout: 3000 }).catch(() => { });
                }
            }
            catch { }
            if (session) {
    ...
    ipcMain.handle("monkey.status", async (_event, payload) => {
    ...
    ipcMain.handle("trace.openInPerfetto", async (_event, payload) => {
        let perfettoWin = null;
        try {
            const filePath = resolveWorkingPath(payload.path);
            const buffer = await readFile(filePath);
...
`;
    ...
    ipcMain.handle("build.stop", async (_event, payload) => {
    ...
    ipcMain.handle("build.status", async (_event, payload) => {
        const session = buildProcesses.get(payload.envId);
        if (!session) {
    ...
    ipcMain.handle("push.start", async (_event, payload) => {
        ...
        const adbCmds = payload.files.map((f) => `adb -s ${payload.deviceId} push "${f.localPath}" "${f.remotePath}"`);
        ...
        const sendPushEvent = (channel, data) => {
            const wins = BrowserWindow.getAllWindows();
            for (const win of wins) {
                try {
    ...
    ipcMain.handle("push.stop", async (_event, payload) => {
        const key = `${payload.envId}:${payload.targetId}`;
    ...
    ipcMain.handle("push.status", async (_event, payload) => {
        const key = `${payload.envId}:${payload.targetId}`;
        const session = pushProcesses.get(key);
        if (!session) {
