...
function buildLogcatSearchRegex(pattern, enabled) {
...
function matchesTextFilter(value, filter, regexEnabled) {
...
function matchesNumericTextFilter(value, filter, regexEnabled) {
    const normalizedFilter = filter.trim();
    if (!normalizedFilter) {
        return true;
    }
...
function matchesLogcatFilterRule(entry, rule, regexEnabled) {
...
function normalizeLogcatCaptureFilters(filters) {
    const normalizedRules = Array.isArray(filters?.rules)
...
function matchesLogcatCaptureFilters(entry, filters) {
...
function buildLogFilePath(session) {
...
function ensureLogFile(session, nextBytes = 0) {
    if (!session.fileStream) {
        openLogFile(session);
        return;
    }
...
async function flushLogFile(session) {
    if (!session.fileStream) {
        return;
    }
...
function writeLogLine(session, line) {
    const text = `${line}\n`;
...
function resetSessionCaptureWindow(session) {
    session.items = [];
    session.partialLine = "";
...
export async function startLogcatSession(deviceId, options) {
    ...
    try {
        ...
        if (options.buffers && options.buffers.length > 0) {
            for (const buf of options.buffers)
                logcatArgs.push("-b", buf);
        }
...
export function updateLogcatSessionFilters(deviceId, filters) {
    const session = sessions.get(deviceId);
    if (!session) {
...
export async function clearLogcatSession(deviceId, filters) {
    const session = sessions.get(deviceId);
    if (!session) {
...
export async function exportLogcatSession(deviceId) {
    const session = sessions.get(deviceId);
