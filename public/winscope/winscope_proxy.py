...
class FileMatcher:
    ...
    def get_filepaths(self, device_id):
        matching_files_raw = call_adb(
            f"shell su root find {self.path} -name {self.matcher}",
            device_id
        )
        ...
        files = [
            line.strip()
            for line in matching_files_raw.split('\n')
            if line.strip()
        ]
...
def call_adb(params: str, device: str = None, stdin: bytes = None):
    log.debug(" call_adb() params=%s", params)
    ...
    try:
        ...
        log.debug("call_adb() output=%r", out)
        return out
...
class FetchFilesEndpoint(DeviceRequestEndpoint):
    def process_with_device(self, server, path, device_id):
        ...
        for f in files:
            ...
            log.debug(
                " FetchFilesEndpoint() Start fetching files for type=%s, paths=%r",
                file_type, file_paths
            )

            for file_path in file_paths:
                # 统一清理掉 Windows 带来的 `\r` 等空白字符
                file_path_clean = file_path.strip()

                with NamedTemporaryFile() as tmp:
                    log.debug(
                        " FetchFilesEndpoint() Fetching file %s from device %s to %s",
                        file_path_clean, device_id, tmp.name
                    )
                    call_adb_outfile(
                        "exec-out su root cat " + file_path_clean,
                        tmp,
                        device_id
                    )

                    size_bytes = os.fstat(tmp.fileno()).st_size
                    log.debug(
                        " FetchFilesEndpoint() Fetched file %s size=%d bytes (type=%s)",
                        file_path_clean, size_bytes, file_type
                    )

                    # 1\) 先 pull 设备上的原始文件用于调试
                    try:
                        debug_dir_device = "./winscope_debug_device"
                        os.makedirs(debug_dir_device, exist_ok=True)

                        safe_name = file_path_clean.replace("/", "_")
                        local_debug_path = os.path.join(debug_dir_device, safe_name)

                        log.debug(
                            " FetchFilesEndpoint() Pulling device file %s to %s",
                            file_path_clean, local_debug_path
                        )
                        pull_cmd = f"pull {file_path_clean} {local_debug_path}"
                        call_adb(pull_cmd, device_id)
                    except AdbError as ex:
                        log.debug(
                            " FetchFilesEndpoint() Failed to pull device file %s: %r",
                            file_path_clean, ex
                        )

                    # 2\) 删除设备上的文件（用 rm -f 简化语法，避免 `; fi'hen` 这类问题）
                    try:
                        log.debug(
                            " FetchFilesEndpoint() Deleting file %s from device %s",
                            file_path_clean, device_id
                        )
                        call_adb(
                            f"shell su root rm -f {file_path_clean}",
                            device_id
                        )
                    except AdbError as ex:
                        log.debug(
                            " FetchFilesEndpoint() Ignoring error when deleting file %s: %r",
                            file_path_clean, ex
                        )

                    # 3\) 读回本地临时文件并写调试副本
                    log.debug(
                        " FetchFilesEndpoint() Uploading file %s (type=%s)",
                        tmp.name, file_type
                    )
                    ...
                    try:
                        debug_dir = "./winscope_debug"
                        os.makedirs(debug_dir, exist_ok=True)
                        timestamp = int(time.time() * 1000)
                        debug_filename = f"{file_type}_{device_id}_{timestamp}{WINSCOPE_EXT}"
                        debug_path = os.path.join(debug_dir, debug_filename)
                        with open(debug_path, "wb") as debug_f:
                            debug_f.write(raw_data)
                        log.debug(
                            " FetchFilesEndpoint() Wrote debug trace to %s (size=%d bytes)",
                            debug_path, len(raw_data)
                        )
                    except Exception as ex:
                        log.debug(
                            " FetchFilesEndpoint() Failed to write debug file for %s: %r",
                            file_path_clean, ex
                        )

                    log.debug(
                        " FetchFilesEndpoint() Local temp file %s read size=%d bytes (type=%s)",
                        tmp.name, len(raw_data), file_type
                    )

                    buf = base64.encodebytes(raw_data).decode("utf-8")
                    log.debug(
                        " FetchFilesEndpoint() Base64 buffer length for %s (type=%s) is %d chars",
                        file_path_clean, file_type, len(buf)
                    )
                    file_buffers[file_type].append(buf)

        if len(file_buffers) == 0:
            log.error(
                " FetchFilesEndpoint() Proxy didn't find any file to fetch for device %s",
                device_id
            )

        j = json.dumps(file_buffers)
        log.debug(
            " FetchFilesEndpoint() Returning buffers for device %s, keys=%r",
            device_id, list(file_buffers.keys())
        )
        server.respond(HTTPStatus.OK, j.encode("utf-8"), "text/json")

def check_root(device_id):
    log.debug(" check_root() device_id=%s", device_id)
    log.debug(" check_root() Checking root access on {}".format(device_id))
    out = call_adb('shell su root id -u', device_id)
    log.debug("check_root() raw output=%r", out)
    return out.strip() == "0"
...
class TraceThread(threading.Thread):
    def __init__(self, device_id, command, stop_command):
        self._keep_alive_timer = None
        self.trace_command = command
        self.stop_command = stop_command  # 新增：记录 stop 命令
    ...
    def end_trace(self):
        if self._keep_alive_timer:
            self._keep_alive_timer.cancel()
        log.debug("Ending trace process on %s", self._device_id)
        terminate_process_cross_platform(self.process, timeout=5)
        try:
    ...
    def run(self):
        ...
        for i in range(50):  # 原来是 10 次，适当拉长等待
            try:
                status = call_adb(
                    "shell su root cat /data/local/tmp/winscope_status",
                    device=self._device_id
                )
                log.debug("winscope_status raw output (try %d)=%r", i, status)
            except AdbError as ex:
                # 文件还没创建或者被删掉时的情况，继续重试
                log.debug("winscope_status read failed on try %d: %r", i, ex)
                status = ""

            if status.strip() == "TRACE_OK":
                try:
                    call_adb(
                        "shell su root rm /data/local/tmp/winscope_status",
                        device=self._device_id
                    )
                except AdbError as ex:
                    log.debug("Failed to remove winscope_status: %r", ex)
                log.debug("Trace finished successfully on {}".format(self._device_id))
                self._success = True
                break

            log.debug("Still waiting for cleanup on {} (status=%r)".format(self._device_id), status)
            time.sleep(0.1)
...
"""
    ...
    def process_with_device(self, server, path, device_id):
        log.debug(" process_with_device() device_id=%s", device_id)
        try:
...
class EndTrace(DeviceRequestEndpoint):
    def _force_stop_on_windows(self, device_id, trace_thread):
        # 在设备上手动执行 stop 命令并写 TRACE_OK
        stop_script = (
            b"set -e\n"
            b"%s\n"
            b'echo "TRACE_OK" > /data/local/tmp/winscope_status\n'
        ) % (trace_thread.stop_command or b"")
        shell = ['adb', '-s', device_id, 'shell']
        log.debug("Windows: running explicit stop script on device %s", device_id)
        try:
            ...
            out, err = process.communicate(stop_script)
            log.debug("Windows stop script out=%r err=%r", out, err)
        except Exception as ex:
            log.error("Windows stop script failed: %r", ex)

    def process_with_device(self, server, path, device_id):
        if device_id not in TRACE_THREADS:
            raise BadRequest("No trace in progress for {}".format(device_id))

        trace_thread = TRACE_THREADS[device_id]

        if trace_thread.is_alive():
            # Windows 上先执行 stop，再结束 trace shell
            if platform.system() == "Windows":
                self._force_stop_on_windows(device_id, trace_thread)
            trace_thread.end_trace()

        success = trace_thread.success()
        out_bytes = trace_thread.out or b""
        err_bytes = trace_thread.err or b""
        out = out_bytes + b"\n" + err_bytes
        command = trace_thread.trace_command
...
class ConfigTrace(DeviceRequestEndpoint):
    def process_with_device(self, server, path, device_id):
        log.debug(" ConfigTrace=>process_with_device() device_id=%s", device_id)
        try:
...
def add_selected_request_to_config(self, server, device_id, config):
    log.debug(" add_selected_request_to_config() device_id=%s", device_id)
    try:
        requested_configs = self.get_request(server)
        for requested_config in requested_configs:
...
def terminate_process_cross_platform(process, timeout=5):
    """在 Unix 上发 SIGINT，在 Windows 上调用 terminate，超时后 kill。"""
    system = platform.system()
    try:
        if system == "Windows":
            # Windows 不支持 SIGINT 信号给普通子进程，用 terminate 即可
            log.debug("Windows: calling terminate() on trace process pid=%s", process.pid)
            process.terminate()
        else:
            # POSIX: 继续使用 SIGINT，让 shell 执行 trap/清理逻辑
            log.debug("POSIX: sending SIGINT to trace process pid=%s", process.pid)
            process.send_signal(signal.SIGINT)

        log.debug("Waiting for trace shell to exit, pid=%s", process.pid)
        process.wait(timeout=timeout)
    except subprocess.TimeoutExpired:
        log.debug("TIMEOUT - sending kill() to trace process pid=%s", process.pid)
        process.kill()
...
class DumpEndpoint(DeviceRequestEndpoint):
    def process_with_device(self, server, path, device_id):
        log.debug(" DumpEndpoint=>process_with_device() device_id=%s", device_id)
        try:
            requested_types = self.get_request(server)
