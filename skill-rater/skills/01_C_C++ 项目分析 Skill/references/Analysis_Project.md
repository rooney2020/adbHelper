...
在目录树之前，必须先补充一段“工程整体功能描述”。这一段不是摘要式一句话说明，而是要回答以下问题：

- 这个工程从系统视角到底承担什么职责。
- 它的主要输入是什么，主要输出是什么。
- 它在上下游系统中的位置是什么，依赖了哪些外部系统或内部模块。
- 它的主运行模式是什么（可执行程序 / SDK / 动态库 / 后台服务 / 工具集等）。
- 它解决的核心业务问题是什么，为什么需要这样一个工程存在。

这段描述必须详细、具体、可验证，禁止写成空泛表述，例如“用于语音处理”“提供基础能力”“负责业务逻辑”等。必须结合 README、构建目标、主入口、对外接口、核心配置和主链路代码给出项目级解释。
...
- 先写“工程整体功能描述”，再按目录树展示结构，最后逐段解释每个目录和关键文件的职责。
- “工程整体功能描述”至少要覆盖：系统职责、输入输出、上下游关系、主运行模式、核心业务目标 5 个方面。
- “工程整体功能描述”必须引用真实入口、真实接口或真实构建目标来支撑，不能只写抽象概念。
...
- **图片嵌入**：流程图生成 PNG 后，必须在成果物 Markdown 对应章节中使用 `![全局执行流程图](png/文件名.png)` 完整嵌入，禁止只写路径或文件名。如果流程图拆分为多张，则每张都必须单独以 `![描述](png/文件名.png)` 嵌入。
...
- **图片嵌入**：类图生成 PNG 后，必须在成果物 Markdown 对应章节中使用 `![类图描述](png/文件名.png)` 完整嵌入，禁止只写路径或文件名。如果类图拆分为多张子图，则每张都必须单独以 `![子图描述](png/文件名.png)` 嵌入。
...
5. **图片嵌入格式（最高优先级）**：成果物 Markdown 中所有图片必须使用完整的 Markdown 图片语法嵌入，格式为 `![图片描述](png/文件名.png)`。**严禁**只给出图片文件路径文本、只写链接 `[文本](路径)` 或只写文件名而不嵌入。每张生成的 PNG 都必须在成果物对应章节中以 `![...](...png)` 的形式完整嵌入，确保在任何 Markdown 渲染器中都能直接显示图片内容而非显示路径文字。
...
17. **工程整体功能描述校验**：检查成果物开头的“工程整体功能描述”是否足够详细、正确，是否明确说明了系统职责、输入输出、上下游关系、主运行模式和核心业务目标。若仅有一句话摘要、没有真实入口/接口/构建目标支撑，或描述与代码主链路不一致，必须补充或修正。18. **图片嵌入校验（必须通过）**：逐一检查成果物中所有生成的 PNG 是否都以 `![描述](png/文件名.png)` 的完整 Markdown 图片语法嵌入。如果发现存在以下任何一种错误形式，必须立即修正：
    - 只有纯文本路径，如 `png/xxx.png`
    - 只有链接形式，如 `[文字](png/xxx.png)` 而不是图片嵌入
    - 只提及文件名但未嵌入
    - 使用了绝对路径而非相对路径
    正确形式必须是 `![有意义的描述文字](png/文件名.png)`，确保渲染时直接显示图片内容。
### 自检输出
