    ...
    try {
        ...
        try {
          ...
          for (const fc of fontCandidates) {
            try {
              ...
              css = css.replace(
                /url\(winscope:\/\/fonts\/MaterialIcons-Regular\.woff2\) format\('woff2'\)/,
      ...
      if (strippedPath === "index.html") {
        ...
        html = html.replace(
          ...
          '<link rel="stylesheet" href="winscope://fonts/roboto.css">'
        );
        html = html.replace(
          /<link[^>]*href="https?:\/\/fonts\.googleapis\.cn\/icon\?family=Material\+Icons"[^>]*>/g,
          '<link rel="stylesheet" href="winscope://fonts/material-icons.css">'
        );
        html = html.replace(
