import { Menu, app, BrowserWindow, dialog, ipcMain, protocol, net, shell, Tray, nativeImage, type OpenDialogOptions } from "electron";
...
let _winscopeProxyProcess: ChildProcess | null = null;
...
async function _checkProxyAlive(): Promise<boolean> {
...
async function _ensureWinscopeProxy(winscopeRoot: string): Promise<string | null> {
  ...
  if (process.platform !== "win32") {
    try {
      ...
      for (const pid of pids) {
        try { process.kill(Number(pid), "SIGTERM"); } catch { /* best-effort */ }
      }
...
let _currentStateDir: string | undefined = undefined;
let _appStateDir: string | undefined = undefined;
function setBackendStateDir(dir: string) { _currentStateDir = dir; }
function resolveStatePath(file: string): string {
...
function registerIpcHandlers() {
  ...
  ipcMain.handle("layout.getWinscopePath", async () => {
    const winscopeRoot = (() => {
      ...
      try { if (statSync(distWinscope).isDirectory()) return distWinscope; } catch {}
      const appDistWinscope = join(app.getAppPath(), "dist", "winscope");
      try { if (statSync(appDistWinscope).isDirectory()) return appDistWinscope; } catch {}
      const bundled = join(app.getAppPath(), "winscope", "dist");
      try { if (statSync(bundled).isDirectory()) return bundled; } catch {}
      const dev = resolve(__dirname, "../../winscope/dist");
      try { if (statSync(dev).isDirectory()) return dev; } catch {}
      return null;
    })();
    if (!winscopeRoot) {
  ...
  ipcMain.handle("layout.winscopeProxy", async () => {
    ...
    if (!winscopeRoot) return { status: "error", message: "未找到 Winscope 目录" };
    try {
  ...
  ipcMain.handle("storage.save", async (_event, payload: { key: string; value: unknown }) => {
    const p = resolveStatePath("app-data.json");
    try {
      await mkdir(dirname(p), { recursive: true });
  ...
  let activeInputRecording: ChildProcess | null = null;
  let inputRecordingBuffer: string[] = [];
  let inputRecordingStartTime = 0;
  let currentRecordingMode: "getevent" | "dumpsys" = "getevent";
  const INPUT_RECORDING_TIMEOUT_MS = 120_000; // max 2 minutes
  const INPUT_RECORDING_MAX_LINES = 100_000;

  // ── Dumpsys polling state ──
  let dumpsysPollTimer: NodeJS.Timeout | null = null;
  let dumpsysSeenEvents: Set<string>;
  let dumpsysRawEvents: Array<{ time: number; type: "touch" | "key"; action: "DOWN" | "UP"; x: number; y: number; keyCode: number }>;
  ...
  function parseDumpsysRecentQueueLine(line: string, seedOnly: boolean = false): void {
    ...
    if (mKey) {
      ...
      const action = mKey[2] as "DOWN" | "UP";
  ...
  function androidKeyCodeToName(code: number): string {
  ...
  function parseDumpsysToSteps(): Array<{ id: string; type: string; name: string; value: string; delayMs: string }> {
    const steps: Array<{ id: string; type: string; name: string; value: string; delayMs: string }> = [];
    let stepIndex = 0;
    ...
    let pendingTouchDown: { x: number; y: number; time: number } | null = null;
   ...
   */
  function buildParseRecentQueueFn(stdout: string | Buffer, seedOnly: boolean = false): void {
  ...
  ipcMain.handle("recording.start", async (_event, payload: { deviceId: string; mode?: "getevent" | "dumpsys" }) => {
    ...
    if (mode === "dumpsys") {
      ...
      async function runPoll() {
        if (!dumpsysPollTimer) return; // stopped
      ...
      proc.stdout?.on("data", (data: Buffer) => {
        if (inputRecordingBuffer.length >= INPUT_RECORDING_MAX_LINES) return;
  ...
  ipcMain.handle("recording.stop", async (_event, _payload: { deviceId: string }) => {
    if (!activeInputRecording && !dumpsysPollTimer) {
      return { status: "error", message: "没有正在进行的录制" };
    }
    let steps: Array<{ id: string; type: string; name: string; value: string; delayMs: string }> = [];
  ...
  function parseGeteventToSteps(rawText: string): Array<{ id: string; type: string; name: string; value: string; delayMs: string }> {
    ...
    let currentTrackingId: number | null = null;
    // frame accumulation
    let frameEvents: Array<{ type: string; code: string; value: string }> = [];
    // Key state
    let pendingKeyDown: { name: string; valueHex: string } | null = null;
      ...
      try {
        ...
        if (oldStat.isFile()) {
          try {
            ...
            if (oldStat.mtimeMs <= newStat.mtimeMs) continue;
