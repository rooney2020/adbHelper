...
function registerIpcHandlers() {
  ...
  let logcatRawEvents: Array<{ type: string; action: string; x: number; y: number; time: number }> = [];
  ...
  function parseLogcatTimestamp(line: string): number {
  ...
  function parseLogcatLine(line: string): { action: "DOWN" | "UP" | "MOVE"; x: number; y: number; time: number } | null {
    ...
    const action = actionName === "DOWN" ? "DOWN" as const : actionName === "UP" ? "UP" as const : actionName === "MOVE" ? "MOVE" as const : null;
  ...
  function parseLogcatToSteps(): Array<{ id: string; type: string; name: string; value: string; delayMs: string }> {
    // Sort by time just in case
    logcatRawEvents.sort((a, b) => a.time - b.time);

    interface StepTime {
      type: string; name: string; value: string;
      startTime: number; // DOWN time (for delay calc)
      endTime: number;   // UP time (for prevEnd calc)
    }
    const stepTimes: StepTime[] = [];
    let pendingDown: { x: number; y: number; time: number } | null = null;
    ...
    for (let i = 0; i < stepTimes.length; i++) {
      ...
      steps.push({
        id: `recorded-${i}`,
        type: s.type as "key" | "tap" | "long_press" | "swipe" | "adb",
  ...
  async function validateLogcatSource(deviceId: string): Promise<string | null> {
    try {
  ...
  ipcMain.handle("recording.start", async (_event, payload: { deviceId: string }) => {
    if (logcatProcess) {
    ...
    proc.stdout?.on("data", (data: Buffer) => {
      if (logcatRawEvents.length >= RECORDING_MAX_EVENTS) return;
  ...
  ipcMain.handle("recording.stop", async (_event, payload: { deviceId: string }) => {
    if (!logcatProcess) {
  ...
  ipcMain.handle("debug.log", async (_event, payload: { message: string }) => {
