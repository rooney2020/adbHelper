...
export function startPerfApiServer(_stateRoot?: string): Promise<number> {
  return new Promise((resolve, reject) => {
...
async function handleApiRoute(req: IncomingMessage, res: ServerResponse, stateDir?: string) {
  ...
  try {
        ...
        if (route === "perf-top-activity") {
          ...
          try {
            ...
            for (const line of lines) {
              if (pkg && line.includes(pkg)) {
                ...
                if (cpuPercent !== null && pid !== null) break;
              }
            }
            if (cpuPercent === null) {
              for (const line of lines) {
                const trimmed = line.trim();
                if (!trimmed || trimmed.startsWith("User") || trimmed.startsWith("System") || trimmed.startsWith("PID")) continue;
                const cpuMatch = trimmed.match(/^\s*([\d.]+)/);
                if (cpuMatch) {
                  const val = Number(cpuMatch[1]);
                  if (!isNaN(val) && val > 0) { cpuPercent = val; break; }
                }
              }
            }
            let totalPss: number | null = null;
            let memRaw = "";
            if (pkg) {
              // Use PID if found (more reliable), otherwise fall back to package name
              let memResult: { stdout: string };
              if (pid) {
              ...
              if (pssMatch) {
                ...
                if (pssMatch2) totalPss = Number(pssMatch2[1]);
              }
          ...
          try {
            ...
            let ioData = { readBytes: null as number | null, writeBytes: null as number | null, readSyscalls: null as number | null, writeSyscalls: null as number | null };
          ...
          try {
            ...
            if (currentEvent.length > 0) crashEvents.push(currentEvent.join("\n"));
